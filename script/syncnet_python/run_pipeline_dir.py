...
parser.add_argument('--video_dir',      type=str, default='input',   help='Input video file');
parser.add_argument('--reference',      type=str, default='',   help='Video reference');
parser.add_argument('--log_file', help='dataset', default='output/input.txt', type=str);
parser.add_argument('--facedet_scale',  type=float, default=0.25, help='Scale factor for face detection');
parser.add_argument('--crop_scale',     type=float, default=0.40, help='Scale bounding box');
parser.add_argument('--min_track',      type=int, default=25,  help='Minimum facetrack duration');
...
def crop_hq_video(opt, track, cropfile):
  flist = glob.glob(os.path.join(opt.frames_dir, opt.reference, '*.jpg'))
  flist.sort()
  ...
  dets['y'] = signal.medfilt(dets['y'], kernel_size=13)
  cs = opt.crop_scale
  lengh = int(dets['s'].max() * 2 * (1 + cs))
  vOut = cv2.VideoWriter(cropfile + 't.avi', fourcc, opt.frame_rate, (lengh, lengh))
  ...
  for fidx, frame in enumerate(track['frame']):
    ...
    vOut.write(cv2.resize(face,(lengh,lengh)))
  audiotmp = os.path.join(opt.tmp_dir, opt.reference, 'audio.wav')
  audiostart = (track['frame'][0]) / opt.frame_rate
  audioend = (track['frame'][-1] + 1) / opt.frame_rate
  ...
  command = ("ffmpeg -y -i %st.avi -i %s -c:v copy -c:a copy %s_hq.avi" % (cropfile, audiotmp, cropfile))
  output = subprocess.call(command, shell=True, stdout=None)
...
video_root = opt.video_dir
videos = os.listdir(video_root)
videos = [x if x.endswith('.mp4') else os.remove(os.path.join(video_root, x))  for x in videos]
videos.sort()
# root_manifest = os.path.join('manifest', identity)
with open(log_file, 'r') as f:
  CompleteID = f.read().split("\n")

for video in videos:
  if video in CompleteID:
    fullPathVideo = os.path.join(video_root, video)
    print(f' video {fullPathVideo} was OK')
  else:
    video_name = video.split('.')[0]
    fullPathVideo = os.path.join(video_root, video)
    print(f'processing video {fullPathVideo}')
    # manifest = os.path.join(root_manifest, video.split('.')[0] + '.text')
    opt.reference = video_name
    opt.videofile = fullPathVideo
    ...
    for ii, track in enumerate(alltracks):
      ...
      crop_hq_video(opt,track,os.path.join(opt.crop_dir,opt.reference,'%05d'%ii))
    ...
    with open(log_file, 'a') as f:
      f.write(f"{video}\n")
    print(f' video {fullPathVideo} OK !!!')
